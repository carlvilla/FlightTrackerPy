...
class RyanairWebScrapper(AirlineWebScrapper):
    ...
    def scrape_airline(self, from_city, to_city, departing_date, returning_date):
        ...
        flight_origin = WebDriverWait(self.driver, 10).until(
            EC.element_to_be_clickable(
                (By.XPATH, '//input[@id="input-button__departure"]')
            )
        )
        flight_destiny = WebDriverWait(self.driver, 10).until(
            EC.element_to_be_clickable(
        ...
        time.sleep(1)
        div_list_places = self.driver.find_elements(
            by="xpath", value='//fsw-airport-item[@class="ng-star-inserted"]'
        )
        ...
        try:
            ...
            logger.debug("Could not click on search button: (%s)", e)
            return False, None

        departing_flights, returning_flights = self.retrieve_all_flights(
        ...
        logger.info("Successful scrapping in Ryanair")
        return self.check_round_flights_under_max_price(round_flight), round_flight

    def set_dates(
        self, departing_date, returning_date, analysed_months=0, pending="departing"
    ):
        ...
        if analysed_months > max_num_months_to_analyse:
            return False
        departing_date_ryaniar_format = datetime.strptime(
            departing_date, "%d/%m/%Y"
        ).strftime("%Y-%m-%d")
        returning_date_ryaniar_format = datetime.strptime(
            returning_date, "%d/%m/%Y"
        ).strftime("%Y-%m-%d")
        element_departing_date_to_select = (
            "//div[@data-id='" + departing_date_ryaniar_format + "']"
        )
        element_returning_date_to_select = (
            "//div[@data-id='" + returning_date_ryaniar_format + "']"
        )
        time.sleep(1.2)
        try:
            if pending == "departing":
                flight_departing_date = self.driver.find_element(
                    by="xpath", value=element_departing_date_to_select
                )
                time.sleep(1.24)
                if (
                    "calendar-body__cell--disabled"
                    in flight_departing_date.get_attribute("class")
                ):
            ...
            WebDriverWait(self.driver, 5).until(
                EC.element_to_be_clickable(
                    (By.XPATH, '//div[@data-ref="calendar-btn-next-month"]')
                )
            ).click()
        ...
        time.sleep(1.35)
        try:
            flight_returning_date = self.driver.find_element(
                by="xpath", value=element_returning_date_to_select
            )
        except Exception:
            logger.info("Dates could not be found in the displayed calendar")
            ...
            return self.set_dates(
                departing_date, returning_date, analysed_months, pending="returning"
            )
        time.sleep(1.332)
        if "calendar-body__cell--disabled" in flight_returning_date.get_attribute(
            "class"
        ):
    ...
    def retrieve_all_flights(self, from_city, to_city, departing_date, returning_date):
        ...
        WebDriverWait(self.driver, 35).until(
            EC.presence_of_element_located(
                (By.XPATH, '//button[normalize-space()="Seleccionar"]')
            )
        )
        # time.sleep(5)
        ...
        flight_cards_departing_flights = flight_list_departing_flights.find_all(
            "flight-card-new"
        )
        departing_flights = []
        for flight_card in flight_cards_departing_flights:
            price = flight_card.find("flights-price-simple").text.translate(
                {ord(c): None for c in string.whitespace}
            )
            hour = flight_card.find(
                "span", {"class": "flight-info__hour"}
            ).text.translate({ord(c): None for c in string.whitespace})
            duration = flight_card.find(
                "div", {"data-ref": "flight_duration"}
            ).text.translate({ord(c): None for c in string.whitespace})
            flight = Flight(from_city, to_city, departing_date, hour, duration, price)
            departing_flights.append(flight)
        flight_cards_returning_flights = flight_list_returning_flights.find_all(
            "flight-card-new"
        )
        returning_flights = []
    ...
    def close_subscriptions_popup(self):
        try:
            ...
            WebDriverWait(self.driver, 5).until(
                EC.element_to_be_clickable(
                    (By.XPATH, '//button[@class="subscriber-widget__mail-wrapper"]')
                )
            ).click()
